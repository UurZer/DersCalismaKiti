...
namespace dersCalismaKiti
{
    ...
    class library: fileClass
    {
        ...
        public string libNotesPath= @" D:\C# Projeleri\dersCalismaKiti\BookNotes.txt";
        public bool flag = false;
        public string mainNoteText;
        public ArrayList MyList = new ArrayList();
        public string bookNumber;
        public int otuz = 30;
        public void writeNotes()
        {
            FileStream fs = new FileStream(libNotesPath, FileMode.Open, FileAccess.ReadWrite);
            ...
            if (strStartReadingDate.Length == 18)
                sw.Write("0" + strStartReadingDate + " - ");
            else
                sw.Write(strStartReadingDate + " - ");
            if (dateProccessStart().ToString().Length == 18)
                sw.Write("0" + dateProccessStart() + "|");
            else
                sw.Write(dateProccessStart() + "|");


            sw.Write(bookName.ToUpper());

            for (int i = 0; i < libEmptyValueProccess(bookName, otuz); i++)
            {
                sw.Write(" ");
            }
            sw.Write("|");
            sw.Write(bookTpye.ToUpper());
            for (int i = 0; i < libEmptyValueProccess(bookTpye, otuz); i++)
            {
                sw.Write(" ");
            }
            sw.Write("|");
            sw.Write(bookNumber);
            for (int i = 0; i < libEmptyValueProccess(bookNumber.ToString(), otuz); i++)
            {
                sw.Write(" ");
            }
            sw.WriteLine("|");
            ...
            sw.WriteLine("-- |-----------------------------------------|------------------------------|------------------------------|------------------------------|");
            sw.Write("Not|");
            int resultText,textLength;
            int rowInt=134;
            textLength = mainNoteText.Length / rowInt;
            resultText = mainNoteText.Length % rowInt;
            if (mainNoteText.Length> rowInt)
            {
                
                for (int i=0;i<(mainNoteText.Length/rowInt);i++)
                {//•	Data.Substring(baslangıçİndex,3)
                    if (textLength-1 == i)
                    {
                        sw.Write(mainNoteText.Substring(textLength * rowInt, resultText));
                        for (int j = 0; j < libEmptyValueProccess(mainNoteText.Substring(textLength * rowInt, resultText), rowInt); j++)
                        {
                            sw.Write(" ");
                        }
                    }
                    ...
                    {
                        sw.WriteLine(mainNoteText.Substring(i* rowInt,rowInt)+"|");
                        sw.Write("---|");
                    }
                }
            }
        ...
        public void libFileProccesRead(string fileWay)
            ...
            for (int i = 0; i < libEmptyValueProccess(howManyDaysProccess().ToString(), otuz); i++)
            {
                sw.Write(" ");
            }
            sw.Write("|");
            sw.Write(totalPages);
            for (int i = 0; i < libEmptyValueProccess(totalPages.ToString(), otuz); i++)
            {
                sw.Write(" ");
            }
            sw.Write("|");
            sw.Write(avarageReadPageProccess());//ortalama günlük okunan sayfa sayısı
            for (int i = 0; i < libEmptyValueProccess(avarageReadPageProccess().ToString(), otuz); i++)
            {
                sw.Write(" ");
            }
            sw.WriteLine("|");
        ...
        public int libEmptyValueProccess(string a,int b)
        {
